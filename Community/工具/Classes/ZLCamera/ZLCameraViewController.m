...
- (void) setup{
    ...
    topView.frame = CGRectMake(0, 0, self.view.zl_width, 60);
...
-(void)Captureimage
{
    ...
    [self.captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         NSData *data = UIImageJPEGRepresentation(t_image, 0.8);
...
- (void)doneAction
{
    ...
    if(self.callback){
        ...
        [self.images enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            UIImageWriteToSavedPhotosAlbum([_images[idx] photoImage], nil, nil, nil);
        }];
    }
    [self cancel:nil];
}
